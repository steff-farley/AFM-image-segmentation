...
is given in this repository (mask4.tif). These are specified on lines 181
and 246.
'''
...
def imgstripe(img):
    ...
    if rand_y + rand_length + 20 > rows - 1:
        ...
        if numofs >= 1:
            img[(rand_x - 15):(rand_x - 14),
            (rand_y + random.randint(0, 10)):(rand_y + rand_length + random.randint(0, 10))] = int
        if numofs >= 2:
            img[(rand_x - 10):(rand_x - 9),
            (rand_y + random.randint(0, 10)):(rand_y + rand_length + random.randint(0, 10))] = int
        if numofs >= 3:
            img[(rand_x - 6):(rand_x - 5),
            (rand_y + random.randint(0, 10)):(rand_y + rand_length + random.randint(0, 10))] = int
        if numofs >= 4:
            img[(rand_x - 1):(rand_x + 1),
            (rand_y + random.randint(0, 10)):(rand_y + rand_length + random.randint(0, 10))] = int
        if numofs >= 5:
            img[(rand_x + 3):(rand_x + 4),
            (rand_y + random.randint(0, 10)):(rand_y + rand_length + random.randint(0, 10))] = int
        if numofs >= 6:
            img[(rand_x + 8):(rand_x + 9),
            (rand_y + random.randint(0, 10)):(rand_y + rand_length + random.randint(0, 10))] = int
        if numofs >= 7:
            img[(rand_x + 13):(rand_x + 14),
            (rand_y + random.randint(0, 10)):(rand_y + rand_length + random.randint(0, 10))] = int
    if rand_y - rand_length - 20 < 0:
        ...
        if numofs >= 1:
            img[(rand_x - 15):(rand_x - 14),
            (rand_y - rand_length - random.randint(0, 10)):(rand_y - random.randint(0, 10))] = int
        if numofs >= 2:
            img[(rand_x - 10):(rand_x - 9),
            (rand_y - rand_length - random.randint(0, 10)):(rand_y - random.randint(0, 10))] = int
        if numofs >= 3:
...
for path in getAllImages(
        path_in):  # get all full file names for all images from path_in and loop through them one by one
    ...
    mask_need_crop = cv2.imread(
        "/home/marmf/Deposition images/Images/For Unet/Training images/Clean only/Masks/Mask4.tif", 0)

    # uncomment if streaking with a high-frequency horizontal banding rather than mask, comment out lines 182-201
