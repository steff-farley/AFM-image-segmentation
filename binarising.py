...
for path in getAllImages('/home/marmf/Deposition images/Images/Testing impact of noise/Originals/'):
    if path.endswith(".png"):
        ...
        ret, img_otsu_ds = cv2.threshold(img_otsu_ds_need_thresh, otsu, 255,
                                         cv2.THRESH_BINARY)  # despeckling leads to small variations in pixel values that make the image not binary, so threshold again
        cv2.imwrite('/home/marmf/Deposition images/Images/Testing impact of noise/Originals/Otsu/' + name + "_otsu.png",
                    img_otsu_ds)

        img_localm = cv2.adaptiveThreshold(img_need_thresh, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 31,
                                           10);  # threshold with local mean
        ...
        cv2.imwrite(
            '/home/marmf/Deposition images/Images/Testing impact of noise/Originals/Localm/' + name + "_localm.png",
            img_localm_ds)
